...
# # ==============================
# # FIX FORMAT JAM (Excel pakai titik)
# # ==============================

# df["Mulai"] = df["Mulai"].astype(str).str.replace(".", ":", regex=False)
# df["Selesai"] = df["Selesai"].astype(str).str.replace(".", ":", regex=False)

# df["Mulai_time"] = pd.to_datetime(df["Mulai"], format="%H:%M:%S", errors="coerce")
# df["Selesai_time"] = pd.to_datetime(df["Selesai"], format="%H:%M:%S", errors="coerce")

# df = df.dropna(subset=["Mulai_time", "Selesai_time"])

# # Gabungkan tanggal + jam
# df["Mulai_dt"] = pd.to_datetime(
#     df["Tanggal"].dt.strftime("%Y-%m-%d") + " " + df["Mulai_time"].dt.strftime("%H:%M:%S")
# )

# df["Selesai_dt"] = pd.to_datetime(
#     df["Tanggal"].dt.strftime("%Y-%m-%d") + " " + df["Selesai_time"].dt.strftime("%H:%M:%S")
# )

# # Handle jika selesai lewat tengah malam
# df.loc[df["Selesai_dt"] < df["Mulai_dt"], "Selesai_dt"] += pd.Timedelta(days=1)

# # Fix parsing jam
# df["Mulai"] = pd.to_datetime(df["Mulai"], errors="coerce").dt.time
# df["Selesai"] = pd.to_datetime(df["Selesai"], errors="coerce").dt.time

# # Convert jam
# df["Mulai"] = pd.to_datetime(df["Mulai"], errors="coerce")
# df["Selesai"] = pd.to_datetime(df["Selesai"], errors="coerce")

# # Buang baris yang jamnya kosong
# # df = df.dropna(subset=["Mulai", "Selesai"])

# # # Gabungkan tanggal + jam
# # df["Mulai_dt"] = pd.to_datetime(
# #     df["Tanggal"].dt.strftime("%Y-%m-%d") + " " + df["Mulai"].dt.strftime("%H:%M:%S")
# # )

# # df["Selesai_dt"] = pd.to_datetime(
# #     df["Tanggal"].dt.strftime("%Y-%m-%d") + " " + df["Selesai"].dt.strftime("%H:%M:%S")
# # )
...
# else:
#     working_df = df[
#         (df["Tanggal"].dt.month == df["Tanggal"].max().month) &
#         (df["Tanggal"].dt.year == df["Tanggal"].max().year)
#     ]
